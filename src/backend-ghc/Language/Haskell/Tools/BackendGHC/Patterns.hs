...
-- trfPattern' p@(LazyPat x pat) = AST.UIrrefutablePat <$> trfPattern (p)
...
-- trfPattern' (SigPat typ pat _) = AST.UTypeSigPat <$> trfPattern pat <*> trfType (hsib_body $ hswc_body typ)
...
trfPatternField' (HsRecField id _ True) = undefined --AST.UFieldPunPattern <$> trfName @n (getFieldOccName id)
