...
-- import GHC.Hs.Syn as GHC
import Name as GHC (Name, occNameString, nameOccName, isSymOcc, occName)
...
trfDecls decls = undefined --addToCurrentScope decls $ makeIndentedListNewlineBefore atTheEnd (mapM trfDecl decls)
...
trfDeclsGroup g@(HsGroup _ vals splices tycls derivs fixities defaults foreigns warns anns rules _)
  = do rdrSpls <- asks declSplices -- now we don't want to rename the splices, just interested in their locations to
       ...
       let (sigs, bagToList -> binds) = undefined --getBindsAndSigs vals
           -- collect the declarations from the group
           alldecls :: [Located (HsDecl n)]
           alldecls = (map (fmap (SpliceD NoExtField)) splices)
                        ++ (map (fmap (ValD NoExtField)) binds)
                        ++ (map (fmap (SigD NoExtField)) sigs)
                        ++ (map (fmap (TyClD NoExtField)) (concat $ map group_tyclds tycls))
                        ++ (map (fmap (DerivD NoExtField)) derivs)
                        ++ (map (fmap (SigD NoExtField . FixSig NoExtField)) (mergeFixityDefs fixities))
                        ++ (map (fmap (DefD NoExtField)) defaults)
                        ++ (map (fmap (ForD NoExtField)) foreigns)
                        ++ (map (fmap (WarningD NoExtField)) warns)
                        ++ (map (fmap (AnnD NoExtField)) anns)
                        ++ (map (fmap (RuleD NoExtField)) rules)
                        ++ (map (fmap (InstD NoExtField)) (hsGroupInstDecls g))
  ...
  where
    ...
    trfDeclsWithScope genNames d = undefined
      -- local (\s -> s {declSplices = []})
...
trfDecl = trfLocNoSema $ \case
  ...
  -- InstD _ (DataFamInstD _ (DataFamInstDecl (hsib_body -> undefined
  -- FamEqn _ con pats _ (HsDataDefn _ nd _ _ _ cons derivs))))
...
trfGADT :: (TransformName n r, n ~ GhcPass p, HasCallStack) => NewOrData -> Located (GHC.IdP n) -> LHsQTyVars n -> Located (HsContext n)
...
trfDataDef :: (TransformName n r, n ~ GhcPass p, HasCallStack) => NewOrData -> Located (GHC.IdP n) -> LHsQTyVars n -> Located (HsContext n)
...
trfSpecializePragma :: forall n r p . (TransformName n r, n ~ GhcPass p, HasCallStack)
                    => Located (GHC.IdP n) -> [Located (HsType n)] -> Activation -> Trf (Ann AST.USpecializePragma (Dom r) RangeStage)
...
trfFieldDecl' (ConDeclField _ names typ _) = undefined -- AST.UFieldDecl <$> (define $ nonemptyAnnList <$> mapM (trfName @n . getFieldOccName) names) <*> trfType typ
...
makeInstanceRuleTyVars :: forall n r p . (TransformName n r, n ~ GhcPass p, HasCallStack) => Located (GHC.IdP n) -> [LHsType n] -> Trf (Ann AST.UInstanceRule (Dom r) RangeStage)
...
trfTypeEq = undefined
  -- trfLocNoSema $ \(FamEqn _ name pats _ rhs)
--   -> AST.UTypeEqn <$> defineTypeVars (focusBefore AnnEqual (combineTypes name pats)) <*> trfType rhs
--   where combineTypes :: Located (GHC.IdP n) -> [LHsType n] -> Trf (Ann AST.UType (Dom r) RangeStage)
...
trfFunDeps :: forall n r . (TransformName n r, HasCallStack)
           => [Located (FunDep (Located (GHC.IdP n)))] -> Trf (AnnMaybeG AST.UFunDeps (Dom r) RangeStage)
...
trfFunDep' :: forall n r . (TransformName n r, HasCallStack) => FunDep (Located (GHC.IdP n)) -> Trf (AST.UFunDep (Dom r) RangeStage)
trfFunDep' (lhs, rhs) = AST.UFunDep <$> trfAnnList ", " (trfName' @n) lhs <*> trfAnnList ", " (trfName' @n) rhs

createDeclHead :: forall n r p . (TransformName n r, n ~ GhcPass p, HasCallStack) => Located (GHC.IdP n) -> GHC.LHsQTyVars n -> Trf (Ann AST.UDeclHead (Dom r) RangeStage)
...
wrapDeclHead :: (TransformName n r, n ~ GhcPass p, HasCallStack) => [GHC.LHsTyVarBndr n] -> Trf (Ann AST.UDeclHead (Dom r) RangeStage) -> Trf (Ann AST.UDeclHead (Dom r) RangeStage)
...
createClassBody :: (TransformName n r, n ~ GhcPass p, HasCallStack) => [GHC.LSig n] -> GHC.LHsBinds n -> [GHC.LFamilyDecl n]
                               -> [GHC.LTyFamInstEqn n] -> Trf (AnnMaybeG AST.UClassBody (Dom r) RangeStage)
...
trfClassElemSig :: forall n r p . (TransformName n r, n ~ GhcPass p, HasCallStack) => Located (GHC.Sig n) -> Trf (Ann AST.UClassElement (Dom r) RangeStage)
trfClassElemSig = trfLocNoSema $ \case
  GHC.TypeSig _ names typ -> AST.UClsSig <$> (annContNoSema $ AST.UTypeSignature <$> define (makeNonemptyList ", " (mapM (trfName @n) names))
                                     <*> trfType (hsib_body $ hswc_body typ))
  GHC.ClassOpSig _ True [name] typ -> AST.UClsDefSig <$> trfName @n name <*> trfType (hsib_body typ)
  GHC.ClassOpSig _ False names typ -> AST.UClsSig <$> (annContNoSema $ AST.UTypeSignature <$> define (makeNonemptyList ", " (mapM (trfName @n) names))
                                              <*> trfType (hsib_body typ))
  GHC.MinimalSig _ _ formula -> AST.UClsMinimal <$> trfMinimalFormula @n formula
  GHC.InlineSig _ name prag -> AST.UClsInline <$> trfInlinePragma @n name prag
  GHC.FixSig _ fixity -> AST.UClsFixity <$> annContNoSema (trfFixitySig fixity)
  s -> unhandledElement "signature in class" s

trfTypeFam :: (TransformName n r, n ~ GhcPass p, HasCallStack) => Located (GHC.FamilyDecl n) -> Trf (Ann AST.UTypeFamily (Dom r) RangeStage)
trfTypeFam = trfLocNoSema trfTypeFam'

trfTypeFam' :: (TransformName n r, n ~ GhcPass p, HasCallStack) => GHC.FamilyDecl n -> Trf (AST.UTypeFamily (Dom r) RangeStage)
trfTypeFam' (GHC.FamilyDecl _ GHC.DataFamily name tyVars _ kindSig _)
  = AST.UDataFamily <$> (case unLoc kindSig of GHC.KindSig _ _ -> between AnnData AnnDcolon; _ -> id) (createDeclHead name tyVars)
                    <*> trfFamilyKind kindSig
trfTypeFam' (GHC.FamilyDecl _ GHC.OpenTypeFamily name tyVars _ kindSig injectivity)
  = AST.UTypeFamily <$> (case unLoc kindSig of GHC.KindSig _ _ -> between AnnType AnnDcolon; _ -> id) (createDeclHead name tyVars)
                   <*> trfFamilyResultSig kindSig injectivity
trfTypeFam' (GHC.FamilyDecl _ (GHC.ClosedTypeFamily {}) _ _ _ _ _) = convertionProblem "trfTypeFam': closed type family received"

trfTypeFamDef :: (TransformName n r, n ~ GhcPass p, HasCallStack) => Located (GHC.TyFamInstEqn n) -> Trf (Ann AST.UClassElement (Dom r) RangeStage)
trfTypeFamDef = undefined
  -- trfLocNoSema $ \(FamEqn _ con pats _ rhs)
  -- -> AST.UClsTypeDef <$> between AnnType AnnEqual (createDeclHead con pats) <*> trfType rhs

trfInstBody :: (TransformName n r, n ~ GhcPass p, HasCallStack) => GHC.LHsBinds n -> [GHC.LSig n] -> [GHC.LTyFamInstDecl n] -> [GHC.LDataFamInstDecl n] -> Trf (AnnMaybeG AST.UInstBody (Dom r) RangeStage)
...
trfClassInstSig :: forall n r p . (TransformName n r, n ~ GhcPass p, HasCallStack) => Located (GHC.Sig n) -> Trf (Ann AST.UInstBodyDecl (Dom r) RangeStage)
trfClassInstSig = trfLocNoSema $ \case
  GHC.TypeSig _ names typ -> AST.UInstBodyTypeSig <$> (annContNoSema $ AST.UTypeSignature <$> makeNonemptyList ", " (mapM (trfName @n) names)
                                           <*> trfType (hsib_body $ hswc_body typ))
  GHC.ClassOpSig _ _ names typ -> AST.UInstBodyTypeSig <$> (annContNoSema $ AST.UTypeSignature <$> define (makeNonemptyList ", " (mapM (trfName @n) names))
                                                <*> trfType (hsib_body typ))
  GHC.SpecInstSig _ _ typ -> AST.USpecializeInstance <$> trfType (hsib_body typ)
  GHC.SpecSig _ name (map hsib_body -> tys) (inl_act -> phase) -> AST.UInstanceSpecialize <$> trfSpecializePragma name tys phase
  GHC.InlineSig _ name prag -> AST.UInlineInstance <$> trfInlinePragma @n name prag
  s -> unhandledElement "class instance signature" s

trfInstTypeFam :: (TransformName n r, n ~ GhcPass p, HasCallStack) => Located (GHC.TyFamInstDecl n) -> Trf (Ann AST.UInstBodyDecl (Dom r) RangeStage)
trfInstTypeFam (L l (GHC.TyFamInstDecl (hsib_body -> eqn))) = copyAnnot AST.UInstBodyTypeDecl (trfTypeEq (L l eqn))

trfInstDataFam :: forall n r p . (TransformName n r, n ~ GhcPass p, HasCallStack) => Located (GHC.DataFamInstDecl n) -> Trf (Ann AST.UInstBodyDecl (Dom r) RangeStage)
trfInstDataFam = undefined
  --  trfLocNoSema $ \case
  ...
  --       <*> makeIndentedList atTheEnd (mapM trfDerivings (unLoc derivs))
  where transformNameAndPats :: Located (GHC.IdP n) -> [GHC.LHsType n] -> Trf (Ann AST.UInstanceHead (Dom r) RangeStage)
...
trfPatternSynonym :: forall n r p . (TransformName n r, n ~ GhcPass p, HasCallStack) => GHC.PatSynBind n n -> Trf (AST.UPatternSynonym (Dom r) RangeStage)
trfPatternSynonym (GHC.PSB _ id lhs def dir) = undefined
  ...
  where trfPatSynLhs :: Located (GHC.IdP n) -> GHC.HsPatSynDetails (Located (GHC.IdP n)) -> AnnKeywordId -> Trf (Ann AST.UPatSynLhs (Dom r) RangeStage)
        trfPatSynLhs id (GHC.PrefixCon args) _
          = annLocNoSema (pure $ foldLocs (getLoc id : map getLoc args)) $ AST.UNormalPatSyn <$> define (trfName @n id) <*> trfAnnList " " (trfName' @n) args
        trfPatSynLhs op (GHC.InfixCon lhs rhs) _
        ...
        trfPatSynLhs id (GHC.RecCon flds) kw
        ...
        trfPatSynRhs :: GHC.HsPatSynDir n -> Located (GHC.Pat n) -> Trf (AST.UPatSynRhs (Dom r) RangeStage)
        trfPatSynRhs GHC.ImplicitBidirectional pat = AST.UBidirectionalPatSyn <$> trfPattern pat <*> nothing " where " "" atTheEnd
        trfPatSynRhs (GHC.ExplicitBidirectional mg) pat = AST.UBidirectionalPatSyn <$> trfPattern pat <*> (makeJust <$> trfPatSynWhere mg)
        trfPatSynRhs GHC.Unidirectional pat = AST.UOneDirectionalPatSyn <$> trfPattern pat

        trfPatSynWhere :: GHC.MatchGroup n (GHC.LHsExpr n) -> Trf (Ann AST.UPatSynWhere (Dom r) RangeStage)
        trfPatSynWhere (GHC.MG { GHC.mg_alts = alts }) = annLocNoSema (pure $ getLoc alts) (AST.UPatSynWhere <$> makeIndentedList (after AnnWhere) (mapM (trfMatch (unLoc id)) (unLoc alts)))

trfFamilyKind :: (TransformName n r, n ~ GhcPass p, HasCallStack) => Located (GHC.FamilyResultSig n) -> Trf (AnnMaybeG AST.UKindConstraint (Dom r) RangeStage)
trfFamilyKind (unLoc -> fr) = case fr of
  GHC.NoSig _ -> nothing "" " " atTheEnd
  GHC.KindSig _ k -> trfKindSig (Just k)
  GHC.TyVarSig _ _ -> convertionProblem "trfFamilyKind: TyVarSig not supported"

trfFamilyResultSig :: forall n r p . (TransformName n r, n ~ GhcPass p, HasCallStack) => Located (GHC.FamilyResultSig n) -> Maybe (GHC.LInjectivityAnn n) -> Trf (AnnMaybeG AST.UTypeFamilySpec (Dom r) RangeStage)
trfFamilyResultSig (L l fr) Nothing = case fr of
  GHC.NoSig _ -> nothing "" " " atTheEnd
  GHC.KindSig _ k -> makeJust <$> (annLocNoSema (pure l) $ AST.UTypeFamilyKind <$> trfKindSig' k)
  GHC.TyVarSig _ tv -> makeJust <$> (annLocNoSema (pure l) $ AST.UTypeFamilyTyVar <$> trfTyVar tv)
trfFamilyResultSig (L _ sig) (Just (L l (GHC.InjectivityAnn n deps)))
  = makeJust <$> (annLocNoSema (pure l) $ AST.UTypeFamilyInjectivity <$> (annContNoSema $ AST.UInjectivityAnn <$> tv <*> trfAnnList ", " (trfName' @n) deps))
    where tv = case sig of GHC.TyVarSig _ tv -> trfTyVar tv
                           _ -> annLocNoSema (pure $ getLoc n) (AST.UTyVarDecl <$> trfName @n n <*> nothing "" "" (pure $ srcSpanEnd (getLoc n)))

trfAnnotationSubject :: forall n r . (TransformName n r, HasCallStack) => SourceText -> GHC.AnnProvenance (GHC.IdP n) -> SrcLoc -> Trf (Ann AST.UAnnotationSubject (Dom r) RangeStage)
trfAnnotationSubject (fromSrcText -> stxt) subject payloadEnd
  = do payloadStart <- advanceStr stxt <$> atTheStart
       case subject of GHC.ValueAnnProvenance name@(L l _) -> annLocNoSema (pure l) (AST.UNameAnnotation <$> trfName @n name)
                       GHC.TypeAnnProvenance name@(L l _) -> annLocNoSema (pure $ mkSrcSpan payloadStart (srcSpanEnd l))
                                                                      (AST.UTypeAnnotation <$> trfName @n name)
                       GHC.ModuleAnnProvenance -> annLocNoSema (pure $ mkSrcSpan payloadStart payloadEnd) (pure AST.UModuleAnnotation)

trfDataKeyword :: GHC.NewOrData -> Trf (Ann AST.UDataOrNewtypeKeyword (Dom r) RangeStage)
trfDataKeyword GHC.NewType = annLocNoSema (tokenLoc AnnNewtype) (pure AST.UNewtypeKeyword)
trfDataKeyword GHC.DataType = annLocNoSema (tokenLoc AnnData) (pure AST.UDataKeyword)
...
trfRewriteRule :: (TransformName n r, n ~ GhcPass p) => Located (GHC.RuleDecl n) -> Trf (Ann AST.URule (Dom r) RangeStage)
trfRewriteRule = undefined
...
trfRuleBndr :: forall n r p . (TransformName n r, n ~ GhcPass p, HasCallStack) => Located (GHC.RuleBndr n) -> Trf (Ann AST.URuleVar (Dom r) RangeStage)
trfRuleBndr = trfLocNoSema $ \case (GHC.RuleBndr _ n) -> AST.URuleVar <$> trfName @n n
                                   (GHC.RuleBndrSig _ n k) -> AST.USigRuleVar <$> trfName @n n <*> trfType (hsib_body $ hswc_body k)

trfMinimalFormula :: forall n r . (TransformName n r, HasCallStack) => Located (BooleanFormula (Located (GHC.IdP n))) -> Trf (Ann AST.UMinimalFormula (Dom r) RangeStage)
...
trfMinimalFormula' :: forall n r . (TransformName n r, HasCallStack) => BooleanFormula (Located (GHC.IdP n)) -> Trf (AST.UMinimalFormula (Dom r) RangeStage)
