...
--   isClass <- undefined -- fromMaybeTM errDefault (isClassTyConNameM con)
...
chkQNameForFC name = undefined
  ...
  -- case mty of
    ...
    --       (preds, tys) = undefined --partition isPredTy . filter (not . isIPPred) $ subTys
    ...
    --       xs = undefined -- mapMaybe getClassPredTys_maybe (thetas ++ preds')
    --       xs' = undefined -- filter (not . isCTupleClass . fst) xs
